...
- **边界行为新约定（2026-02）**：
  - 长按移动到边界后应“停住”，不触发边界循环/跨区跳转
  - 仅 `KeyDownEvent`（单击）可触发边界循环或退出到其他区域
  - `KeyRepeatEvent` 在边界必须 `handled`，防止系统默认焦点搜索介入
- 典型支持点：
...
### 5.4 直播 `live_tab.dart`

- 直播房间网格：
  - 首列左移 -> 侧边栏
  - 首行上移 -> 直播分类 tab
  - 末行下移 -> `LoadMore`（存在时）或阻止
- 顶部分类 tab（水平）：
  - 首项左键 -> 侧边栏（不循环）
  - 末项右键 -> 循环到首项
  - 长按到左右边界后停住；继续单击才触发上述边界动作
...
3. 水平/垂直 tab 边界循环必须区分事件类型：
   - `KeyDownEvent`：允许边界循环（如末项右键回首项）
   - `KeyRepeatEvent`：在边界直接 `handled`（停住）
4. 首项左键若用于“回侧边栏”，该行为应优先于水平循环（不做左向循环）
5. 弹窗/侧面板用 `FocusScope` 拦截方向键，避免 key repeat 穿透到底层页面
6. 对关键入口（如播放器全局 handler）统一处理 `KeyRepeatEvent`，避免长按时落到系统默认行为
...
- 约定：
  1. 长列表必须使用 `itemExtent`，禁止仅依赖“估算高度 + GlobalKey”；
  2. 焦点变化时按 `index * itemExtent` 计算目标偏移，确保目标项进入可视区；
  3. 首次打开面板与 tab 切换后，使用无动画定位（`jumpTo`）先对齐，再进入正常导航。
- 原因：
  - `ListView.builder` 对屏外项是懒构建，目标项未构建时 `ensureVisible` 可能无效；
  - 一旦估算高度与实际行高不一致（尤其在 textScaler != 1 时），会出现“焦点索引已变化但屏幕内无焦点”的错位现象。

## 8. 改动检查单

1. 新页面是否明确了四方向边界行为（首行/末行/首列/末列）
2. 是否区分 `KeyDownEvent` 与 `KeyRepeatEvent`（边界长按应停住）
...
- 侧边栏（垂直）：
  ...
  - 长按上下到边界后停住
  - 右键行为取决于标签切换策略：
    - **聚焦即切页**：焦点移到哪个图标就切换到该 tab，右键进入当前聚焦 tab 的内容
    - **确认键切页**：焦点移动仅高亮图标不切 tab，右键始终进入当前**选中（可见）** tab 的内容区（与 YouTube 等主流 TV App 一致）
  - 所有侧边栏项的右键均由显式 handler 处理（`blockRight: onMoveRight == null`），不依赖系统默认焦点搜索
- 页面内顶部 tab（水平）：
  - 首项左键优先回侧边栏（如首页分类 tab、设置分类 tab、直播分类 tab）
  - 末项右键可循环到首项
  - 边界长按停住，单击才触发边界动作
...
- 焦点问题排查优先级：
  1. 先确认“焦点索引是否变化”（逻辑层）；
  2. 再确认“焦点项是否进入可视区”（滚动层）；
  3. 最后确认“焦点样式是否可见”（视觉层）。
- 视觉层约定：
  - 浅色模式禁止使用固定深色/浅色硬编码（如 `Colors.white`/`Colors.black`）作为主要文本色；
  - 焦点底色统一走 `AppColors.navItemSelectedBackground`，文本走语义色 token；
  - 边框/左侧指示线若使用主题色，需配合足够对比度的背景（避免“有焦点但看不见”）。
- 回归风险提醒：
  - 若只修滚动不修颜色，仍会出现“看起来像焦点不在屏内”的误判；
  - 若只修颜色不修滚动，仍会出现真实的屏外焦点问题。
