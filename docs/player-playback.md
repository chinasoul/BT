...
在 `VideoPlayer.java`（ExoPlayer 基类）中，采用 **init 阶段静音 + 首次 play() 延迟恢复** 策略：

1. 构造函数中 `prepare()` 之前调用 `exoPlayer.setVolume(0f)`
2. `play()` 首次调用时，post 150ms 延迟后恢复 `targetVolume`（默认 1.0）
3. `setVolume()` 在静音期间仅更新 `targetVolume`，不写入 ExoPlayer
4. `dispose()` 时清除未执行的 Handler 回调，防止 use-after-release
